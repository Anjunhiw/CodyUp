...
router.get('/cart/:user_id', async (req, res) => {
  ...
  try {
    ...
    const rows = await conn.query(`
      SELECT c.cart_id, c.item_origin_id, i.item_name, i.item_price, c.cart_option
      FROM cart c
...
router.post('/cart/add', async (req, res) => {
  const { user_id, item_origin_id, cart_option, price } = req.body;

  if (!user_id || !item_origin_id || !cart_option || !price) {
  ...
  try {
    ...
    // 중복 여부 확인 (옵션 전체 비교를 위해 JSON stringify)
    const existing = await conn.query(
      `SELECT * FROM cart 
       WHERE user_id = ? AND item_origin_id = ? AND cart_option = ?`,
      [user_id, item_origin_id, cart_option]
    );

    if (existing.length > 0) {
      // 동일 옵션 상품이 이미 있음 → 수량 누적은 선택사항 (이번엔 새 row로 추가 유지)
      conn.release();
      return res.status(200).json({ success: true, message: '이미 장바구니에 있습니다.' });
    }
    ...
    await conn.query(
      `INSERT INTO cart (user_id, item_origin_id, price, cart_option)
       VALUES (?, ?, ?, ?)`,
      [user_id, item_origin_id, price, cart_option]
    );
  ...
  try {
    ...
    for (const item of order_items) {
      ...
      const itemRows = await conn.query(
        'SELECT item_name, item_option, item_price FROM item WHERE item_origin_id = ?',
        [item_origin_id]
      );
      ...
      const itemPrice = itemRows[0].item_price;
      let itemOption;
      try {
